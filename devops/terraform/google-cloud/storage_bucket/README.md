...
```json
{
    "config": {
        ...
        "bucket_settings": {
            "prefix": "Prefijo para nombres de bucket",
            "suffix": "Sufijo para nombres de bucket"
        }
    }
}
```
    ...
    "buckets": [
        {
            ...
            "location": "Ubicación donde estará disponible",
            "force_destroy": "Booleano para especificar si se borraran todos los objetos contenidos en el bucket al momento de borrar este",
            "versioning": "Booleano para especificar si se versionaran los archivos dentro del bucket"
        },,...
    ]
}
```
...
-   **init**, prepara el directorio de trabajo para poder ejecutar los otros comandos. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/storage_bucket init -backend-config=ubicacion_configuracion_backend`
-   **plan**, muestra los cambios del recurso que se van a ejecutar. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/storage_bucket plan -var-file=ubicacion_tfvars`
-   **apply**, actualiza los cambios en el recurso. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/storage_bucket apply -var-file=ubicacion_tfvars`
-   **destroy**, elimina los recursos especificados. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/storage_bucket destroy -var-file=ubicacion_tfvars`
