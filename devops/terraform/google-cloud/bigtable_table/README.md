...
```json
{
    "config":{
        ...
        "zone": "Zona donde se va a crear la instancia/tabla de bigtable, ex. us-central1-a",
        "credentials_path": "Ruta al archivo de credenciales para autenticar con Google Cloud",
        "definitions_path": "ruta al directorio que contiene las definiciones de las instancias y tablas de bigtable"
    }
}
```
    ...
    "bigtable_instances": [
        {
            ...
            "deletion_protection": "Permitir que un terraform destroy pueda destruirlo, true o false",
            "storage_type": "Tipo de almacenamiento, SDD o HDD",
            "project_id": "ID de proyecto donde se va a crear la instancia",
            "zone": "Zona donde va a correr la instancia, ex. us-central1-a",
            "cluster_id": "ID para el cluster",
            "min_nodes": "Numero minimo de nodos en la instancia",
            "max_nodes": "Numero maximo de nodos a los que la instancia puede escalar",
            "cpu_target": "Porcentaje de CPU para que escale"
        }
    ],
    "bigtable_tables": [
        {
            "name": "Nombre de la tabla",
            "project_id": "ID del proyecto donde se va a correr el recurso tabla",
            "deletion_protection": "Permitir que un terraform desroy pueda destruirlo, PROTECTED o UNPROTECTED",
            "instance_name": "Nombre referencia a la instancia a la que va a pertenecer esta tabla",
            "column_family": "Nombre de Familia para agrupar columnas"
        }
    ]
}
```
...
-   **init**, prepara el directorio de trabajo para poder ejecutar los otros comandos. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigtable_table init -backend-config=ubicacion_configuracion_backend`
-   **plan**, muestra los cambios del recurso que se van a ejecutar. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigtable_table plan -var-file=ubicacion_tfvars`
-   **apply**, actualiza los cambios en el recurso. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigtable_table apply -var-file=ubicacion_tfvars`
-   **destroy**, elimina los recursos especificados. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigtable_table destroy -var-file=ubicacion_tfvars`
