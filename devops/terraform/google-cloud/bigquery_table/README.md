...
```json
{
    "config": {
        ...
        "definitions_path": "ruta al directorio que contiene las definiciones de las tablas",
        "bigquery_settings": {
            "prefix": "Prefijo para nombres de bigquery",
            "suffix": "Sufijo para nombres de bigquery"
        }
    }
}
```
    ...
    "tables": [
        {
            ...
            "labels": "Estructura json que contiene los labels para una tabla, solo permite minusculas, numeros y guiones, en cualquiera de los keys y values, de lo contrario marcará error"
        },...
    ]
}
```
...
-   **init**, prepara el directorio de trabajo para poder ejecutar los otros comandos. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigquery_table init -backend-config=ubicacion_configuracion_backend`
-   **plan**, muestra los cambios del recurso que se van a ejecutar. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigquery_table plan -var-file=ubicacion_tfvars`
-   **apply**, actualiza los cambios en el recurso. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigquery_table apply -var-file=ubicacion_tfvars`
-   **destroy**, elimina los recursos especificados. Ejemplo: `TF_DATA_DIR=/path/.terraform terraform -chdir=./devops/terraform/google-cloud/bigquery_table destroy -var-file=ubicacion_tfvars`
